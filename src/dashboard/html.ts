...
export function renderDashboardHtml(stats: any): string {
  const coolingHtml = (stats.cooling_sources || []).length > 0
    ? (stats.cooling_sources || []).map((s: any) =>
        `<div style="background:#fff3e0;padding:8px;margin:4px 0;border-radius:4px">
